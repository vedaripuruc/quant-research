...
# Configurable bar size in minutes (default 60 = 1H)
BAR_MINUTES = int(sys.argv[1]) if len(sys.argv) > 1 else 60
...
def process_hour_file(
    ...
    """Process one hourly tick file, building sub-bars of BAR_MINUTES size."""
    bar_interval_ms = BAR_MINUTES * 60 * 1000
    hour_start_ms = dt_to_ms(hour_start)

    # Accumulate sub-bars: keyed by bar_start_ms
    sub_bars: dict[int, dict] = {}
    file_fills = 0
    ...
    with gzip.open(file_path, "rt", encoding="utf-8") as handle:
        for raw_line in handle:
            ...
            # Process orders/trades on every tick
            ...
            # Bucket tick into sub-bar
            offset_ms = ts_ms - hour_start_ms
            bar_index = max(0, offset_ms // bar_interval_ms)
            bar_start_ms = hour_start_ms + bar_index * bar_interval_ms

            if bar_start_ms not in sub_bars:
                sub_bars[bar_start_ms] = {
                    "open": price, "high": price, "low": price, "close": price,
                    "volume": 0.0, "tick_count": 0,
                }

            sb = sub_bars[bar_start_ms]
            if price > sb["high"]:
                sb["high"] = price
            if price < sb["low"]:
                sb["low"] = price
    ...
    # Convert to Bar objects, sorted by time
    completed_bars: list[Bar] = []
    for bar_start_ms in sorted(sub_bars.keys()):
        sb = sub_bars[bar_start_ms]
        if sb["tick_count"] == 0:
            continue
        completed_bars.append(Bar(
            timestamp=datetime.fromtimestamp(bar_start_ms / 1000, tz=timezone.utc),
            open=sb["open"],
            high=sb["high"],
            low=sb["low"],
            close=sb["close"],
            volume=sb["volume"],
            tick_count=sb["tick_count"],
        ))

    return completed_bars, file_fills, file_cancellations
...
def main() -> None:
    ...
    print(f"Bar size: {BAR_MINUTES}min | Scanning {DATA_START.date()} to {DATA_END.date()} ({total_hours} hourly files)")
    ...
    for hour_start in iter_hours(DATA_START, DATA_END):
        ...
        new_bars, fills, cancellations = process_hour_file(
            file_path=file_path,
        ...
        if files_processed % 100 == 0:
            print(f"Processed {files_processed} files... ({hour_start:%Y-%m-%d %H:00} UTC) | {len(bars)} bars, {len(closed_trades)} trades")
    ...
    trades_csv_path = RESULTS_DIR / f"fvg_tick_{BAR_MINUTES}min_trades.csv"
    summary_json_path = RESULTS_DIR / f"fvg_tick_{BAR_MINUTES}min_summary.json"

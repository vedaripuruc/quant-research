#!/usr/bin/env python3
"""
FVG Wall backtest using 1H OHLC for detection and tick data for execution.
"""
...
def backtest_fvg_wall(
    ...
    for fvg in fvgs:
        ...
        if fvg.fvg_type == "bullish":
            sl = fvg.middle_high + sl_buffer_pips * PIP_SIZE
            risk = sl - entry_price
            tp = entry_price - rr_ratio * risk
        else:
            sl = fvg.middle_low - sl_buffer_pips * PIP_SIZE
            risk = entry_price - sl
            tp = entry_price + rr_ratio * risk
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backtest FVG Wall strategy.")

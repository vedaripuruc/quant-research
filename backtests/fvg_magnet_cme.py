#!/usr/bin/env python3
"""
FVG Magnet with CME Session Filter.
Only considers FVGs that FORMED during CME/NYSE active hours.

CME Session (NY time): 8:30 AM - 4:00 PM ET
In UTC: 13:30-21:00 (winter EST) / 12:30-20:00 (summer EDT)
Conservative window: 13:00-21:00 UTC to cover both
"""
...
# CME Session hours in UTC (conservative window covering EST/EDT)
CME_START_UTC = 13  # 13:00 UTC = 8:00 AM ET (winter) / 9:00 AM ET (summer)
CME_END_UTC = 21    # 21:00 UTC = 4:00 PM ET (winter) / 5:00 PM ET (summer)
...
def is_cme_session(dt: datetime) -> bool:
    """Check if datetime falls within CME active session (UTC)."""
    hour = dt.hour
    return CME_START_UTC <= hour < CME_END_UTC
...
def find_fvgs(df: pd.DataFrame, cme_only: bool = True) -> List[FVG]:
    """Find FVGs, optionally filtering to CME session only."""
    fvgs: List[FVG] = []
    for i in range(2, len(df)):
        ...
        # FVG forms at the close of the 3rd candle
        formed_at = pd.to_datetime(right["timestamp"]).to_pydatetime() + timedelta(hours=1)
        
        # Filter: only FVGs formed during CME session
        if cme_only and not is_cme_session(formed_at):
            continue
...
def backtest_fvg_magnet(
    ...
    """FVG Magnet with optional CME session filter."""
    trades: List[Trade] = []
    fvgs = find_fvgs(df, cme_only=cme_only)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FVG Magnet with CME session filter.")
    ...
    parser.add_argument("--no-cme-filter", action="store_true", help="Disable CME session filter")
    args = parser.parse_args()

    df = pd.read_csv(args.ohlc, parse_dates=["timestamp"])
    tick_index = build_tick_index(Path(args.tick_dir))

    cme_only = not args.no_cme_filter
    filter_label = "CME ONLY" if cme_only else "ALL SESSIONS"
    ...
    print(
        f"[{filter_label}] Trades={stats['trades']} Wins={stats['wins']} Losses={stats['losses']} "

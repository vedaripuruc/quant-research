...
def zone_remaining_gap(zone: FVGZone) -> float:
    return zone.current_upper - zone.current_lower


def is_zone_active(zone: FVGZone, ts: pd.Timestamp) -> bool:
    return zone.active_from <= ts and zone_remaining_gap(zone) >= PIP_SIZE


def apply_partial_fill(zone: FVGZone, bar_high: float, bar_low: float) -> None:
    """
    Shrink FVG edges as price trades through the zone.
    - Bullish FVG shrinks from above (upper edge moves down via bar low)
    - Bearish FVG shrinks from below (lower edge moves up via bar high)
    """
    if zone.direction == "bullish":
        if bar_low < zone.current_upper:
            zone.current_upper = max(zone.current_lower, min(zone.current_upper, bar_low))
        return

    if bar_high > zone.current_lower:
        zone.current_lower = min(zone.current_upper, max(zone.current_lower, bar_high))
...
def trade_from_signal(
    ...
    if reaction == "bounce_from_above" and zone.direction == "bullish":
        signal_type = "fade"
        side = "long"
        sl = zone.current_lower
    elif reaction == "bounce_from_below" and zone.direction == "bearish":
        signal_type = "fade"
        side = "short"
        sl = zone.current_upper
    elif reaction == "clean_break_up":
        signal_type = "break"
        side = "long"
        sl = (zone.current_lower + zone.current_upper) / 2.0
    ...
    for i in range(len(hourly) - 1):
        ...
        for zone in active_zones:
            reaction = classify_reaction(
                ...
                bar_low=bar_low,
                bar_close=bar_close,
                lower=zone.current_lower,
                upper=zone.current_upper,
            )
        ...
        for zone in active_zones:
            apply_partial_fill(zone, bar_high=bar_high, bar_low=bar_low)

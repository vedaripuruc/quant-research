#!/usr/bin/env python3
"""
FVG Magnet backtest - RIDE the gap instead of fading it.
Hypothesis: FVG acts as a black hole that pulls price through.

Wall (fade): Bullish FVG → SHORT | Bearish FVG → LONG
Magnet (ride): Bullish FVG → LONG | Bearish FVG → SHORT
"""
...
class Trade:
    ...
    exit_time: datetime
    exit_price: float
    outcome: str  # win, loss, timeout, no_touch
    pnl_pips: float
...
def backtest_fvg_magnet(
    ...
    """
    MAGNET strategy: Ride the FVG, don't fade it.
    - Bullish FVG → LONG (expect price to continue up through gap)
    - Bearish FVG → SHORT (expect price to continue down through gap)
    """
    trades: List[Trade] = []
    fvgs = find_fvgs(df)
    ...
    for fvg in fvgs:
        ...
        # Entry trigger: price touches the FVG zone
        for ts, price in iter_ticks(tick_index, start_time, end_time):
            if fvg.fvg_type == "bullish":
                # Price coming down into bullish FVG from above → LONG
                if last_price is not None and last_price > fvg.top and price <= fvg.top:
                    entry_time = ts
                    entry_price = price
                    break
            else:
                # Price coming up into bearish FVG from below → SHORT
        ...
        # MAGNET: Ride WITH the gap direction
        if fvg.fvg_type == "bullish":
            # LONG: SL below gap, TP above
            sl = fvg.bottom - sl_buffer_pips * PIP_SIZE
            risk = entry_price - sl
            tp = entry_price + rr_ratio * risk
        else:
            # SHORT: SL above gap, TP below
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backtest FVG Magnet strategy (ride the gap).")
    ...
    parser.add_argument("--min-gap", type=float, default=5.0, help="Minimum FVG size in pips.")
    parser.add_argument("--max-gap", type=float, default=100.0, help="Maximum FVG size in pips.")
    parser.add_argument("--sl-buffer", type=float, default=2.0, help="SL buffer in pips.")
